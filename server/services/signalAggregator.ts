...
import { runWeakSignalScoring } from "./weakSignal/conceptScorer";
import { runTrendClustering } from "./weakSignal/trendClusterer";
...
if (signalsForExtraction.length > 0) {
  ...
  await writeEdgeSnapshots(signal, conceptNodeIds);
      ...
      // Run weak signal scoring after concept extraction and graph updates
      if (signalsForExtraction.length > 0) {
        console.log("[weakSignal] running weak signal scoring...");
        const scoringStats = await runWeakSignalScoring();
        console.log("[weakSignal] scoring completed:", {
          conceptsScored: scoringStats.conceptsScored,
          durationMs: scoringStats.durationMs,
          topConcept: scoringStats.topConcepts[0]?.concept || "none",
        });

        // Run trend clustering after scoring
        console.log("[weakSignal] running trend clustering...");
        const clusteringResult = await runTrendClustering();
        console.log("[weakSignal] clustering completed:", {
          clustersCreated: clusteringResult.clustersCreated,
          conceptsClustered: clusteringResult.conceptsClustered,
          durationMs: clusteringResult.durationMs,
          topCluster: clusteringResult.topClusters[0]?.label || "none",
        });
      }
} catch (err) {
